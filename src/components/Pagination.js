...
export function Pagination() {
  ...
  const { currentPage } = useSelector(state => state.paginationRed);
  ...
  const goToPage = (n) => {
    const onNewPage =(filtered) ? filtered.slice(maxPerPage * n, maxPerPage * (n + 1))
      : data.slice(maxPerPage * n, maxPerPage * (n + 1));
